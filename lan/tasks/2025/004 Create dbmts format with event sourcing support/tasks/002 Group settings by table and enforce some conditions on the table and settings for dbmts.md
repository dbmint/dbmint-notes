...
Spawned in: [<a name="spawn-task-a09a05" />^spawn-task-a09a05](../004%20Create%20dbmts%20format%20with%20event%20sourcing%20support.md#spawn-task-a09a05)
...
1. No duplicate `#[derive(EventSourcing)]` settings exist for the same table
1. Natural order, in which `#[derive(EventSourcing)]` is always followed by `#[es(latest|sum)]` settings for the same table.
1. All columns expect for `id` and `obj_id` have the `#[es(latest|sum)]` setting for all tables with the `#[derive(EventSourcing)]` setting.
1. All tables with the `#[derive(EventSourcing)]` setting must have a non-null primary key integer `id` and non-null integer `obj_id`.
1. `id` and `obj_id` must have no `#[es(latest|sum)]` settings.
...
![Pasted image 20251009180337.png](../../../../../attachments/Pasted%20image%2020251009180337.png)

Hmm, I guess I can't implement `TryFrom` to give many in this way.

![Pasted image 20251009180446.png](../../../../../attachments/Pasted%20image%2020251009180446.png)
...
Spawn [000 Preserve a last value and use it in iteration mapping](../howtos/000%20Preserve%20a%20last%20value%20and%20use%20it%20in%20iteration%20mapping.md) <a name="spawn-howto-ecf5e1" />^spawn-howto-ecf5e1

2025-10-09 Wk 41 Thu - 20:30 +03:00

![Pasted image 20251009203033.png](../../../../../attachments/Pasted%20image%2020251009203033.png)

![Pasted image 20251009203040.png](../../../../../attachments/Pasted%20image%2020251009203040.png)
...
![Pasted image 20251009204248.png](../../../../../attachments/Pasted%20image%2020251009204248.png)
...
Came from [003 Impl writing the expanded event sourcing output given the grouped settings by table for dbmts](003%20Impl%20writing%20the%20expanded%20event%20sourcing%20output%20given%20the%20grouped%20settings%20by%20table%20for%20dbmts.md) for testing!
